...
const RED = "#C8102E", DRED = "#9B0020", AMB = "#D97706";
...
const CICONS = {
  "Mini Eats":"🥖","Roast Chicken":"🍗","Shawarma":"🌯","Submarine":"🥙",
  "Burgers":"🍔","Specials":"⭐","Rotti Kottu":"🫕","Cheese Kottu":"🧀",
  "String Kottu":"🍝","Kottu":"🥘","Starters":"🥗","Fried Rice":"🍚"
};
...
const SCRIPT_CODE = `function doPost(e) {
  try {
    ...
    if (sh.getLastRow() === 0) {
      sh.appendRow(['Order#','Type','Ref','Date',
    ...
    d.orders.forEach(function(o) {
      ...
      sh.appendRow([o.orderNum, o.type, o.ref,
        o.date, o.time, items, o.total,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const openNew = () => {
      ...
      setTillSess(s);
      localStorage.setItem("cr_till_sess", JSON.stringify(s));
    };
  ...
  const addItem = item => {
    ...
    if(mode === "table") {
      updTbl(selTbl, t => {
        const c = ex ? t.cart.map(i => i.id===item.id ? {...i,qty:i.qty+1} : i) : [...t.cart, {...item,qty:1}];
  ...
  const adj = (id, d) => {
    if(mode === "table") {
      updTbl(selTbl, t => {
        const c = t.cart.map(i => i.id===id ? {...i,qty:i.qty+d} : i).filter(i => i.qty > 0);
        return {...t, cart:c, openedAt: c.length ? t.openedAt : null};
      });
    } else {
      updTA(selTA, t => ({...t, cart: t.cart.map(i => i.id===id ? {...i,qty:i.qty+d} : i).filter(i => i.qty > 0)}));
    }
  };
  ...
  const newTakeaway = () => {
    const id = taCounter;
    const newTA = {id, taNum:taLabel(id), cart:[], createdAt:new Date().toISOString()};
    const nl = [...takeaways, newTA];
    setTakeaways(nl); saveTAs(nl);
    const nc = taCounter + 1; setTaCounter(nc);
    localStorage.setItem("cr_tacounter", String(nc));
  ...
  const finalize = (method, paid) => {
    ...
    const order = {
      orderNum: counter,
      type: isTbl ? "Dine In" : "Takeaway",
      ref:  isTbl ? "Table " + selTbl : (ta ? ta.taNum : "TA"),
      tableId: isTbl ? selTbl : null,
      taId:   !isTbl ? selTA  : null,
      date: d.toLocaleDateString("en-GB"),
      time: d.toLocaleTimeString("en-GB", {hour:"2-digit",minute:"2-digit"}),
      items: cart.map(i => ({...i})),
      total, paymentMethod:method, amountPaid:paid,
  ...
  const doCloseTill = () => {
    ...
    const iMap  = {};
    dayOrders.forEach(o => o.items.forEach(i => { iMap[i.name] = (iMap[i.name] || 0) + i.qty; }));
    const gross  = dayOrders.reduce((s,o) => s + o.total, 0);
    const cashRv = cashO.reduce((s,o) => s + o.total, 0);
    const report = {
      date: tod, openedAt: closed.openedAt, closedAt,
      openingBalance:0, totalOrders: dayOrders.length,
      dineInCount: dineO.length, dineInRev: dineO.reduce((s,o)=>s+o.total,0),
      takeawayCount: taO.length, takeawayRev: taO.reduce((s,o)=>s+o.total,0),
      grossSales: gross, cashCount: cashO.length, cashRev: cashRv,
      cardCount: cardO.length, cardRev: cardO.reduce((s,o)=>s+o.total,0),
      closingCashBalance: cashRv,
      topItems: Object.entries(iMap).sort((a,b)=>b[1]-a[1]).slice(0,5),
    };
    setTillSess(closed); saveTillSess(closed);
    const nh = [...tillHist, {...closed, report}];
  ...
  const printWin = html => {
    ...
    w.document.write("<!DOCTYPE html><html><head><meta charset='utf-8'><title>Chef Rooster</title><style>"
      + "*{box-sizing:border-box;margin:0;padding:0}"
      + "body{font-family:'Segoe UI',sans-serif;padding:20px;font-size:13px;color:#1a1a1a}"
      + "h1{font-size:18px;font-weight:800;letter-spacing:2px;color:#C8102E}"
      + ".dash{border-top:2px dashed #ccc;margin:10px 0}"
      + ".row{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #f0f0f0;align-items:center}"
  ...
  const printOrder = o => {
    const isTbl = o.type === "Dine In", ac = isTbl ? "#C8102E" : "#d97706";
    const badge = isTbl ? "TABLE " + o.tableId + " — DINE IN" : "TAKEAWAY — " + o.ref;
    const rows  = o.items.map(it =>
      "<div class='row'><span>" + it.name + " <span style='color:#bbb'>x" + it.qty + "</span></span>"
      + "<span style='font-weight:700'>Rs. " + (it.price*it.qty).toLocaleString() + "</span></div>"
    ).join("");
    ...
    printWin(
      ...
      + "<div style='font-size:26px'>🐓</div><h1>CHEF ROOSTER</h1>"
      + "<div style='font-size:11px;margin-top:6px;background:" + (isTbl?"#fff3f5":"#fffbeb") + ";color:" + ac + ";padding:3px 12px;border-radius:6px;display:inline-block;font-weight:700'>" + badge + "</div>"
      + "<div style='font-size:11px;color:#999;margin-top:6px'>Order #" + o.orderNum + " · " + o.date + " " + o.time + "</div></div>"
      ...
      + "<div style='text-align:center;margin-top:14px;padding-top:12px;border-top:2px dashed #ccc;font-size:10px;color:#ccc;line-height:2'>★ Thank you ★<br>Chef Rooster · Sri Lanka</div>"
    );
  };
  ...
  const printDailyReport = () => {
    ...
    const cashO = dayO.filter(o => o.paymentMethod === "cash");
    const cardO = dayO.filter(o => o.paymentMethod === "card");
    const dineO = dayO.filter(o => o.type === "Dine In");
    const taO   = dayO.filter(o => o.type === "Takeaway");
    const gross  = dayO.reduce((s,o) => s+o.total, 0);
    const cashRv = cashO.reduce((s,o) => s+o.total, 0);
    const cardRv = cardO.reduce((s,o) => s+o.total, 0);
    const iMap   = {};
    dayO.forEach(o => o.items.forEach(i => { iMap[i.name] = (iMap[i.name]||0) + i.qty; }));
    ...
    const orderRows = dayO.slice().reverse().map(o =>
      "<div class='row'><span style='font-size:12px'><b>#" + o.orderNum + "</b> " + o.ref + " &nbsp;" + o.time + "</span>"
      + "<span style='display:flex;gap:6px;align-items:center'>"
      + "<span class='badge' style='background:" + (o.type==="Dine In"?"#fff3f5":"#fffbeb") + ";color:" + (o.type==="Dine In"?"#C8102E":"#d97706") + "'>" + o.type + "</span>"
      + "<b style='color:#C8102E'>Rs." + o.total.toLocaleString() + "</b></span></div>"
    ).join("");
    printWin(
      "<div style='text-align:center;padding-bottom:12px;border-bottom:2px dashed #ccc;margin-bottom:14px'>"
      + "<div style='font-size:26px'>🐓</div><h1>CHEF ROOSTER</h1>"
      + "<div style='font-size:12px;font-weight:700;color:#555;margin-top:4px;letter-spacing:1px'>DAILY SALES REPORT</div>"
      + "<div style='font-size:11px;color:#999;margin-top:6px'>" + tod + "</div></div>"
      ...
      + (top5.length ? "<div style='font-weight:700;font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px'>Top Items</div>" + topRows + "<div class='dash'></div>" : "")
      + "<div style='font-weight:700;font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px'>All Orders (" + dayO.length + ")</div>"
      + (orderRows || "<div style='color:#ccc;text-align:center;padding:10px'>No orders today</div>")
      + "<div style='text-align:center;margin-top:16px;padding-top:10px;border-top:2px dashed #ccc;font-size:10px;color:#ccc;line-height:2'>"
  ...
  const sync = async () => {
    ...
    setSyncMsg("⏳ Syncing…");
    try {
      const r = await fetch(sheetUrl, {method:"POST", body:JSON.stringify({orders:q})});
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(145deg,#9B0020,#C8102E,#e63950)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',system-ui,sans-serif",padding:20}}>
      <div style={{background:"#fff",borderRadius:24,padding:"40px 36px",width:"100%",maxWidth:380,boxShadow:"0 24px 60px rgba(0,0,0,0.25)"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{width:72,height:72,borderRadius:20,background:"linear-gradient(135deg,#C8102E,#9B0020)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:38,margin:"0 auto 16px"}}>🐓</div>
          <div style={{fontSize:22,fontWeight:800,letterSpacing:2,color:RED}}>CHEF ROOSTER</div>
          <div style={{fontSize:12,color:"#aaa",marginTop:4,letterSpacing:1}}>POINT OF SALE SYSTEM</div>
        </div>
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,fontWeight:700,color:"#555",marginBottom:6,letterSpacing:0.5,textTransform:"uppercase"}}>Username</div>
        ...
        <div style={{marginBottom:20}}>
          <div style={{fontSize:11,fontWeight:700,color:"#555",marginBottom:6,letterSpacing:0.5,textTransform:"uppercase"}}>Password</div>
          <div style={{position:"relative"}}>
            <input value={lPass} onChange={e=>{setLPass(e.target.value);setLErr("");}} onKeyDown={e=>e.key==="Enter"&&doLogin()}
        ...
        <div style={{textAlign:"center",marginTop:20,paddingTop:16,borderTop:"1px solid #f0f0f0",fontSize:11,color:"#d0d0d0",letterSpacing:0.5}}>
          CHEF ROOSTER · SRI LANKA
        </div>
      </div>
    </div>
  );
  ...
  if(!loaded) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",fontFamily:"'Segoe UI',sans-serif",color:"#aaa",fontSize:14}}>
      Loading…
    </div>
  );

  // Shared clear modal (rendered inside each screen)
  const ClearModal = () => !clrModal ? null : (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}}>
      <div style={{background:"#fff",borderRadius:22,padding:"34px 28px",maxWidth:320,width:"100%",margin:16,textAlign:"center",boxShadow:"0 24px 60px rgba(0,0,0,0.3)"}}>
        ...
        <input type="password" value={clrPass} onChange={e=>{setClrPass(e.target.value);setClrErr("");}}
          onKeyDown={e=>e.key==="Enter"&&doClr()} placeholder="Enter manager password" autoFocus
  ...
  // ══════════════════════════════════════════════════════
  if(screen === "payment") {
    const paid = parseFloat(cashAmt) || 0;
    ...
    const cardFlow = () => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => finalize("card", total), 1600);
      }, 2600);
    };
    return (
      <div style={P.page}>
        <TopBar onBack={()=>setScreen("pos")} right={<span style={{color:"rgba(255,255,255,0.85)",fontSize:13}}>{ref} &nbsp;·&nbsp; Due: <b style={{color:"#fff",fontSize:15}}>{Rs(total)}</b></span>}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 16px",gap:18,overflowY:"auto"}}>
          ...
          {payStep === "cash" && (
            <div style={{...P.card,width:"100%",maxWidth:380,padding:22}}>
              ...
              <button onClick={()=>paid>=total&&finalize("cash",paid)} disabled={paid<total}
          ...
          {payStep === "card-done" && (
            <div style={{...P.card,width:"100%",maxWidth:380,padding:48,textAlign:"center"}}>
              ...
              <p style={{color:"#999",fontSize:13,margin:0}}>Generating receipt…</p>
            </div>
          )}
        </div>
      </div>
    ...
    return (
      <div style={P.page}>
        ...
        <div className="np" style={{...P.topbar,background:"linear-gradient(135deg,"+ac+","+(isTbl?DRED:"#b45309")+")",justifyContent:"space-between"}}>
          <span style={{color:"#fff",fontWeight:700,fontSize:14}}>🐓 Receipt — Order #{lastOrder.orderNum} · {lastOrder.ref}</span>
        ...
        <div style={{flex:1,overflowY:"auto",background:"#f4f4f4",display:"flex",justifyContent:"center",padding:24,alignItems:"flex-start"}}>
          <div className="pa" style={{...P.card,maxWidth:400,width:"100%",padding:30}}>
            <div style={{textAlign:"center",borderBottom:"2px dashed #e0e0e0",paddingBottom:20,marginBottom:20}}>
              <div style={{fontSize:34,marginBottom:4}}>🐓</div>
              <div style={{fontSize:21,fontWeight:800,letterSpacing:2,color:RED}}>CHEF ROOSTER</div>
              ...
              <div style={{fontSize:11,color:"#aaa",marginTop:8}}>Order #{lastOrder.orderNum} · {lastOrder.date} {lastOrder.time}</div>
            </div>
            ...
            <div style={{borderTop:"2px dashed #e0e0e0",marginTop:16,paddingTop:16}}>
              ...
              {lastOrder.paymentMethod === "cash" && <>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:13}}>
                  <span style={{color:"#888"}}>Cash Received</span>
                  <span style={{fontWeight:600}}>{Rs(lastOrder.amountPaid)}</span>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:13}}>
                  <span style={{color:"#888"}}>Change Due</span>
                  <span style={{fontWeight:600,color:"#16a34a"}}>{Rs(lastOrder.change)}</span>
                </div>
              </>}
            ...
            <div style={{textAlign:"center",marginTop:22,paddingTop:18,borderTop:"2px dashed #e0e0e0",fontSize:11,color:"#ccc",lineHeight:2.2}}>
              ★ {isTbl?"Enjoy your meal!":"Your order is being prepared!"} ★<br/>Chef Rooster · Sri Lanka
            </div>
          </div>
        </div>
      </div>
  ...
  if(screen === "reports") {
    ...
    return (
      <div style={P.page}>
        ...
        {tcModal && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}}>
            <div style={{background:"#fff",borderRadius:22,padding:"34px 28px",maxWidth:340,width:"100%",margin:16,textAlign:"center",boxShadow:"0 24px 60px rgba(0,0,0,0.3)"}}>
              ...
              <div style={{fontWeight:800,fontSize:17,color:"#1a1a1a",marginBottom:4}}>Close Till — {tillSess?.date}</div>
              <div style={{fontSize:12,color:"#999",marginBottom:20}}>This will lock the till and generate the Day End Report. Enter password to confirm.</div>
              <input type="password" value={tcPass} onChange={e=>{setTcPass(e.target.value);setTcErr("");}}
                onKeyDown={e=>e.key==="Enter"&&doCloseTill()} placeholder="Manager password" autoFocus
            ...
            <div style={{background:"#fff",borderRadius:20,maxWidth:440,width:"100%",overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,0.3)"}}>
              ...
              <div style={{padding:"24px 28px",maxHeight:"75vh",overflowY:"auto"}}>
                <div style={{textAlign:"center",borderBottom:"2px dashed #e0e0e0",paddingBottom:18,marginBottom:18}}>
                  ...
                  <div style={{fontSize:18,fontWeight:800,letterSpacing:2,color:RED}}>CHEF ROOSTER</div>
                  <div style={{fontSize:13,fontWeight:700,color:"#555",marginTop:4,letterSpacing:1}}>DAY END REPORT</div>
                  <div style={{fontSize:12,color:"#aaa",marginTop:6}}>{dayEnd.date}</div>
                </div>
                ...
                <div style={{fontWeight:800,fontSize:12,color:"#aaa",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Sales Summary</div>
                {[
                  ["Dine-In Orders ("+dayEnd.dineInCount+")",   Rs(dayEnd.dineInRev),   RED],
                  ["Takeaway Orders ("+dayEnd.takeawayCount+")",Rs(dayEnd.takeawayRev),"#d97706"],
                  ["Total Orders ("+dayEnd.totalOrders+")",     Rs(dayEnd.grossSales),  "#1a1a1a"],
                ].map(([l,v,col]) => (
                ...
                <div style={{fontWeight:800,fontSize:12,color:"#aaa",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Payment Breakdown</div>
                {[
                  ["💵 Cash Payments ("+dayEnd.cashCount+")", Rs(dayEnd.cashRev)],
                  ["💳 Card Payments ("+dayEnd.cardCount+")", Rs(dayEnd.cardRev)],
                ].map(([l,v]) => (
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #f5f5f5",fontSize:13}}>
                ...
                {dayEnd.topItems.length > 0 && <>
                  <div style={{fontWeight:800,fontSize:12,color:"#aaa",letterSpacing:1,textTransform:"uppercase",margin:"16px 0 8px"}}>Top Items</div>
        ...
        <TopBar onBack={()=>setScreen("home")} right={<button onClick={sync} style={{...P.tbBtn,background:"rgba(255,255,255,0.22)"}}>☁️ Sync</button>}>
        ...
        <div style={{display:"flex",background:"#fff",borderBottom:"2px solid #eee",flexShrink:0}}>
          {[["📊","Summary","summary"],["💰","Till Management","till"]].map(([ic,lb,tab]) => (
        ...
        {syncMsg && <div style={{padding:"9px 18px",background:"#fff",borderBottom:"1px solid #eee",fontSize:13}}>{syncMsg}</div>}
          ...
          <div style={{flex:1,overflowY:"auto",padding:14}}>
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}>
              <button onClick={printDailyReport} style={{background:"linear-gradient(135deg,#C8102E,#9B0020)",border:"none",borderRadius:10,color:"#fff",cursor:"pointer",fontWeight:700,fontSize:13,padding:"9px 18px",display:"flex",alignItems:"center",gap:7}}>
                🖨️ Print Daily Sales Report
              </button>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(138px,1fr))",gap:10,marginBottom:14}}>
              {[
                ["🧾","Today's Orders",  todayO.length,false,RED],
                ["💰","Today's Revenue", Rs(todayRev),  true, RED],
                ["🍽️","Dine-In Revenue", Rs(dineRev),   false,RED],
                ["🛍️","Takeaway Revenue",Rs(taRev),     false,AMB],
                ["📦","Takeaway Orders", taCount,        false,AMB],
                ["📈","All-Time Revenue",Rs(totalRev),   true, RED],
              ].map(([ic,lb,val,big,col]) => (
                <div key={lb} style={{background:"#fff",borderRadius:12,padding:14,textAlign:"center",boxShadow:"0 1px 8px rgba(0,0,0,0.06)",borderTop:"3px solid "+col}}>
                  <div style={{fontSize:22,marginBottom:6}}>{ic}</div>
                  <div style={{fontSize:big?14:20,fontWeight:800,color:col,lineHeight:1.2}}>{val}</div>
                  <div style={{fontSize:10,color:"#aaa",marginTop:4,textTransform:"uppercase",letterSpacing:0.3}}>{lb}</div>
                </div>
              ))}
            </div>
            {top5.length > 0 && (
                ...
                <div key={o.orderNum} style={{padding:"9px 0",borderBottom:"1px solid #f5f5f5",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      ...
                      <span style={{fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:6,background:o.type==="Dine In"?"#fff3f5":"#fffbeb",color:o.type==="Dine In"?RED:AMB}}>
                        {o.type==="Dine In"?"🍽️ Dine In":"🛍️ Takeaway"}
                      </span>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <button onClick={()=>printOrder(o)} title="Print this order"
                      style={{width:28,height:28,borderRadius:8,border:"1px solid #eee",background:o.type==="Dine In"?"#fff3f5":"#fffbeb",cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      🖨️
                    </button>
                  </div>
                </div>
            ...
            <div style={{...P.card,marginBottom:14,borderTop:"4px solid "+(tillOpen?"#22c55e":"#94a3b8")}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                <div>
                  ...
                  <div style={{fontSize:20,fontWeight:800,color:"#1a1a1a"}}>{tillSess?.date || "—"}</div>
                </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
                {[
                  ["🕐 Opened At",      tillSess?.openedAt || "—"],
                  ["🔒 Closed At",      tillSess?.closedAt || "—"],
                  ["💵 Opening Balance","Rs. 0.00"],
                  ["💰 Cash in Till",   Rs(liveCash)],
                ].map(([l,v]) => (
              ...
              {tillOpen ? (
                <button onClick={()=>{setTcPass("");setTcErr("");setTcModal(true);}}
                ...
                <div>
                  <div style={{background:"#f1f5f9",borderRadius:10,padding:"12px 16px",textAlign:"center",marginBottom:10}}>
                    ...
                    <div style={{fontSize:11,color:"#aaa",marginTop:3}}>A new till will open automatically tomorrow morning</div>
                  </div>
                ...
                {tillHist.slice().reverse().slice(0,10).map((s,i) => (
                  <div key={i} style={{padding:"10px 0",borderBottom:"1px solid #f5f5f5"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                      <div>
                        <span style={{fontSize:13,fontWeight:700,color:"#1a1a1a"}}>{s.date}</span>
                    ...
                    {s.report && <div style={{fontSize:11,color:"#aaa"}}>{s.report.totalOrders} orders · Opened {s.openedAt} · Closed {s.closedAt} · Cash {Rs(s.report.cashRev)}</div>}
                  </div>
                ))}
              </div>
            )}
            {!tillHist.length && (
              <div style={{textAlign:"center",padding:"30px 20px",color:"#ddd"}}>
                <div style={{fontSize:40}}>📅</div>
                <div style={{fontSize:13,marginTop:10,color:"#ccc"}}>No previous sessions yet</div>
              </div>
            )}
          </div>
        )}
  ...
  if(screen === "settings") {
    return (
      <div style={P.page}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:14}}>
          <div style={{...P.card,marginBottom:12}}>
            <p style={{margin:"0 0 4px",fontWeight:700,fontSize:14}}>☁️ Google Sheets Endpoint</p>
            <p style={{margin:"0 0 14px",fontSize:12,color:"#999"}}>Paste your Apps Script Web App URL to enable daily sync</p>
            ...
            <button onClick={()=>{ localStorage.setItem("cr_url",sheetUrl); setSyncMsg("✅ Saved"); setScreen("reports"); }}
              style={{...P.btn,width:"100%",padding:13}}>Save & Go to Reports →</button>
          </div>
          <div style={{...P.card,marginBottom:12}}>
            <p style={{margin:"0 0 10px",fontWeight:700,fontSize:14}}>📋 Apps Script Setup Guide</p>
            <div style={{fontSize:12,color:"#555",lineHeight:2,marginBottom:14}}>
          ...
          <div style={{...P.card,marginBottom:12}}>
            ...
            <button onClick={()=>{setLoggedIn(false);setLUser("");setLPass("");setScreen("home");setHomeTab("tables");}}
              style={{...P.btn,background:"#475569",width:"100%",padding:13}}>Sign Out</button>
          </div>
          <div style={{...P.card,marginBottom:12}}>
            <p style={{margin:"0 0 10px",fontWeight:700,fontSize:14}}>🔄 Reset</p>
            <button onClick={()=>{ if(window.confirm("Reset all tables?")){ const f=mkTables(); setTables(f); localStorage.setItem("cr_tables",JSON.stringify(f)); }}}
              style={{...P.btn,background:AMB,width:"100%",padding:12,marginBottom:8}}>🪑 Reset All Tables</button>
            <button onClick={()=>{ if(window.confirm("Clear all active takeaway orders?")){ setTakeaways([]); localStorage.setItem("cr_takeaways","[]"); }}}
              style={{...P.btn,background:"#059669",width:"100%",padding:12}}>🛍️ Clear Active Takeaways</button>
          </div>
          <div style={P.card}>
            <p style={{margin:"0 0 10px",fontWeight:700,fontSize:14,color:"#dc2626"}}>⚠️ Danger Zone</p>
            <button onClick={()=>{ if(window.confirm("Delete ALL order history? Cannot be undone.")){ setOrders([]); setCounter(1); localStorage.setItem("cr_orders","[]"); localStorage.setItem("cr_counter","1"); }}}
              style={{...P.btn,background:"#dc2626",width:"100%",padding:13}}>🗑️ Clear All Order History</button>
          </div>
        </div>
      </div>
    );
  ...
  if(screen === "pos") {
    ...
    const cartLabel = isTbl ? "Table " + selTbl : ref;
    return (
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div style={{...P.topbar,background:gradBg}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div>
                <div style={{color:"#fff",fontWeight:800,fontSize:16,letterSpacing:1}}>CHEF ROOSTER</div>
                <div style={{color:"rgba(255,255,255,0.65)",fontSize:10,letterSpacing:1}}>{isTbl ? "TABLE " + selTbl + " — DINE IN" : "TAKEAWAY — " + ref}</div>
              </div>
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <div style={{background:"rgba(255,255,255,0.15)",borderRadius:8,padding:"4px 12px",fontSize:11,color:"#fff",fontWeight:700}}>{isTbl?"🍽️":"🛍️"} {ref}</div>
              <button onClick={()=>{setSyncMsg("");setScreen("reports");}} style={P.tbBtn}>📊 Reports</button>
            </div>
          </div>
          <div style={{background:"#fff",borderBottom:"1px solid #eee",overflowX:"auto",display:"flex",flexShrink:0,scrollbarWidth:"none"}}>
            {Object.keys(MENU).map(c => (
              <button key={c} onClick={()=>setCat(c)}
                style={{padding:"10px 13px",border:"none",background:"transparent",cursor:"pointer",whiteSpace:"nowrap",fontSize:11,fontWeight:700,color:cat===c?ac:"#888",borderBottom:"3px solid "+(cat===c?ac:"transparent"),transition:"color .12s"}}>
                {CICONS[c]} {c}
              </button>
            ))}
          </div>
            ...
            {MENU[cat].map(item => {
              ...
              return (
                <button key={item.id} onClick={()=>addItem(item)}
                  style={{border:"2px solid "+(inCart?ac:"#e8e8e8"),borderRadius:14,padding:"13px 9px 11px",background:"#fff",cursor:"pointer",textAlign:"left",position:"relative",boxShadow:inCart?"0 4px 14px "+ac+"25":"0 1px 5px rgba(0,0,0,0.05)"}}>
                  ...
                  <div style={{fontSize:9,color:"#c0c0c0",marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>{cat}</div>
                  <div style={{fontSize:12,fontWeight:700,color:"#1a1a1a",lineHeight:1.3,marginBottom:8}}>{item.name}</div>
                  <div style={{fontSize:14,fontWeight:800,color:ac}}>Rs. {item.price.toLocaleString()}</div>
                </button>
              );
            })}
          </div>
        </div>
        <div style={{width:285,background:"#fff",borderLeft:"1px solid #ececec",display:"flex",flexDirection:"column",flexShrink:0}}>
          <div style={{padding:"12px 14px 10px",borderBottom:"1px solid #f2f2f2",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontWeight:700,fontSize:14,color:"#1a1a1a"}}>{cartLabel} Order</div>
              <div style={{fontSize:11,color:"#bbb"}}>{count} item{count!==1?"s":""}{openedAt ? " · " + elapsed(openedAt) : ""}</div>
            </div>
            {cart.length > 0 && (
              <button
                onClick={() => { const lbl = cartLabel + " — All Items"; askClear(lbl, clearCart); }}
          ...
          <div style={{flex:1,overflowY:"auto",padding:"8px 10px"}}>
            {!cart.length ? (
              <div style={{textAlign:"center",padding:"36px 16px",color:"#e0e0e0"}}>
                <div style={{fontSize:44}}>{isTbl?"🍽️":"🛍️"}</div>
                <div style={{fontSize:12,marginTop:10}}>Tap items to add<br/>to {ref}</div>
              </div>
            ) : cart.map(item => (
              <div key={item.id} style={{display:"flex",alignItems:"center",gap:6,marginBottom:7,padding:"9px 10px",background:"#f9f9f9",borderRadius:11}}>
                ...
                <div style={{fontSize:11,fontWeight:700,color:ac,minWidth:50,textAlign:"right",flexShrink:0}}>Rs.{(item.price*item.qty).toLocaleString()}</div>
              </div>
            ))}
          </div>
          <div style={{padding:13,borderTop:"1px solid #f2f2f2"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:13}}>
              ...
              <span style={{fontSize:24,fontWeight:800,color:ac}}>Rs. {total.toLocaleString()}</span>
            </div>
            <button onClick={()=>{setPayStep("choose");setCashAmt("");setScreen("payment");}} disabled={!cart.length}
              style={{width:"100%",padding:15,background:cart.length?gradBg:"#e8e8e8",border:"none",borderRadius:12,cursor:cart.length?"pointer":"not-allowed",fontSize:15,fontWeight:800,color:"#fff",letterSpacing:0.4}}>
              💳 Proceed to Pay
            </button>
          </div>
        </div>
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#f3f3f3",fontFamily:"'Segoe UI',system-ui,sans-serif",overflow:"hidden"}}>
      ...
      <div style={P.topbar}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{color:"#fff",fontWeight:800,fontSize:18,letterSpacing:2}}>CHEF ROOSTER</div>
            <div style={{color:"rgba(255,255,255,0.55)",fontSize:9,letterSpacing:1.5}}>ORDER MANAGEMENT</div>
          </div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <div style={{background:"rgba(255,255,255,0.12)",borderRadius:8,padding:"4px 10px",textAlign:"center"}}>
            <div style={{color:"rgba(255,255,255,0.6)",fontSize:9,letterSpacing:0.5}}>LIVE REVENUE</div>
            <div style={{color:"#fff",fontWeight:800,fontSize:13}}>Rs.{(liveTableRev+liveTARev).toLocaleString()}</div>
          </div>
          <button onClick={()=>{setSyncMsg("");setScreen("reports");}} style={P.tbBtn}>📊 Reports</button>
          <button onClick={()=>setScreen("settings")} style={P.tbBtn}>⚙️ Settings</button>
        </div>
      </div>
      <div style={{display:"flex",background:"#fff",borderBottom:"2px solid #eee",flexShrink:0}}>
        {[["🍽️","Dine In","tables"],["🛍️","Takeaway","takeaway"]].map(([ic,lb,tab]) => (
          <button key={tab} onClick={()=>setHomeTab(tab)}
            style={{flex:1,padding:"13px 0",border:"none",background:"transparent",cursor:"pointer",fontSize:14,fontWeight:700,
              ...
              borderBottom: "3px solid " + (homeTab===tab ? (tab==="tables"?RED:AMB) : "transparent"),
              transition:"all .15s"}}>
            {ic} {lb}
      ...
      {homeTab === "tables" && <>
        ...
        <div style={{flex:1,overflowY:"auto",padding:12,display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(152px,1fr))",gap:10,alignContent:"start"}}>
          {tables.map(t => {
            const occ  = t.cart.length > 0;
            const tTot = t.cart.reduce((s,i) => s+i.price*i.qty, 0);
            const tCnt = t.cart.reduce((s,i) => s+i.qty, 0);
            const time = elapsed(t.openedAt);
            return (
              <div key={t.id} style={{background:"#fff",border:"2px solid "+(occ?RED:"#e5e5e5"),borderRadius:16,overflow:"hidden",boxShadow:occ?"0 4px 16px "+RED+"20":"0 1px 5px rgba(0,0,0,0.05)"}}>
                <button onClick={()=>{setSelTbl(t.id);setMode("table");setCat(Object.keys(MENU)[0]);setScreen("pos");}}
                  style={{width:"100%",padding:"15px 13px 10px",cursor:"pointer",textAlign:"left",background:"transparent",border:"none"}}>
                  <div style={{position:"relative"}}>
                    <div style={{position:"absolute",top:0,right:0,width:9,height:9,borderRadius:"50%",background:occ?RED:"#22c55e"}}/>
                    <div style={{fontSize:10,color:"#c0c0c0",textTransform:"uppercase",letterSpacing:1,marginBottom:5,fontWeight:600}}>Table</div>
                    <div style={{fontSize:34,fontWeight:900,color:occ?RED:"#d5d5d5",lineHeight:1,marginBottom:8,letterSpacing:-1}}>{t.id}</div>
                    {occ ? (
                      <div>
                ...
                {occ && (
                  <button onClick={()=>askClear("Table " + t.id, ()=>clearTableFromHome(t.id))}
      ...
      {homeTab === "takeaway" && <>
        <div style={{padding:"12px 14px",background:"#fff",borderBottom:"1px solid #eee",flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={newTakeaway} style={{background:"linear-gradient(135deg,#D97706,#b45309)",border:"none",borderRadius:10,color:"#fff",cursor:"pointer",fontWeight:700,fontSize:13,padding:"9px 18px"}}>
            + New Takeaway Order
          </button>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:12}}>
          {!takeaways.length && (
            <div style={{textAlign:"center",padding:"60px 20px",color:"#ddd"}}>
              ...
              <div style={{fontSize:12,marginTop:6,color:"#ddd"}}>Tap "New Takeaway Order" to start</div>
              <button onClick={newTakeaway} style={{marginTop:20,background:"linear-gradient(135deg,#D97706,#b45309)",border:"none",borderRadius:12,color:"#fff",cursor:"pointer",fontWeight:700,fontSize:14,padding:"13px 28px"}}>
                + New Takeaway Order
              </button>
            </div>
          )}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10,alignContent:"start"}}>
            {takeaways.map(ta => {
              const tTot = ta.cart.reduce((s,i) => s+i.price*i.qty, 0);
              const tCnt = ta.cart.reduce((s,i) => s+i.qty, 0);
              const time = elapsed(ta.createdAt);
              return (
                <div key={ta.id} style={{background:"#fff",border:"2px solid "+AMB,borderRadius:16,overflow:"hidden",boxShadow:"0 4px 18px "+AMB+"20"}}>
                  <button onClick={()=>{setSelTA(ta.id);setMode("takeaway");setCat(Object.keys(MENU)[0]);setScreen("pos");}}
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                      <div>
                        ...
                        <div style={{fontSize:24,fontWeight:900,color:AMB,letterSpacing:-0.5}}>{ta.taNum}</div>
                      </div>
